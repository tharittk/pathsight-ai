...
class PathSightApp(tk.Tk):
    ...
    def _build_controls(self, parent: ttk.Frame) -> None:
        ...
        info_text = "Instructions: < TBC >\n"
        ttk.Label(sec_info, text=info_text, justify=tk.LEFT, wraplength=320).grid(
            row=0, column=0, sticky="w"
        )

        # ── Section: Selected wells & distributions ─────────────────────
        sec_sel = ttk.LabelFrame(
            parent, text="Selected Wells & Distributions", padding=10
        )
        ...
        self.tree = ttk.Treeview(sec_sel, columns=cols, show="tree headings", height=12)
        ...
        scrollbar = ttk.Scrollbar(sec_sel, orient="vertical", command=self.tree.yview)
        self.tree.configure(yscrollcommand=scrollbar.set)
        self.tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        # ── Section: Run button ─────────────────────────────────────────
        btn_run = ttk.Button(parent, text="▶  Run Optimisation", command=self._on_run)
    ...
    def _draw_view(self) -> None:
        ...
        if self.grid is not None and self.grid.n_active > 0:
            ...
            def _scatter_grid_band(
                mask: np.ndarray, color: str, label: str, alpha: float, size: float
            ) -> None:
            ...
            _scatter_grid_band(
                (gv >= 0.6) & (gv < 0.9), CLR_ANTI_DIP_SIDE, "Anti-dip side", 0.35, 5
            )
    ...
    def _on_set_wellhead(self) -> None:
        ...
        try:
            ...
            messagebox.showerror(
                "Invalid input", "Please enter numeric X and Y values."
            )
            return
    ...
    def _on_run(self) -> None:
        ...
        if self.wellhead_xy is None:
            messagebox.showwarning("No wellhead", "Set the wellhead location first.")
            return
        ...
        wx, wy = self.wellhead_xy
        self.well_path = generate_vertical_well_path(
            wx, wy, max_depth=4000.0, step=10.0
        )
        self.status_var.set(
